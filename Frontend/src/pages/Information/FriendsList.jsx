  ...
  useEffect(() => {
    const fetchFriends = async () => {
      try {
        ...
        const transformedFriends = response.data.friends.map(friend => ({
          id: friend.userId,
          name: `${friend.firstName} ${friend.lastName}`,
          imageUrl: friend.profilePicture?.src && friend.profilePicture.src !== "" 
          ? `Picture/${friend.profilePicture.src}` 
          ...
          friends.map(friend => (
            <Link to={`/${friend.id}`} key={friend.id}>
              <div className={styles.friendcard}>
                <img
               src={`/public/img/${friend.imageUrl}`}
                  alt={friend.name}
