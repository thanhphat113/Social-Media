...
const FollowersList = () => {
  const [followers, setFollowers] = useState([]); // Đổi tên biến từ friends thành followers
  ...
  useEffect(() => {
    const fetchFollowers = async () => {
      try {
        const response = await axios.get('http://localhost:5164/api/Information/followers', {
          withCredentials: true,
        });

        // Xử lý dữ liệu followers
        const transformedFollowers = response.data.map(follower => ({
          id: follower.userId,
          name: `${follower.firstName} ${follower.lastName}`,
          imageUrl: follower.profilePicture?.src || 
          (follower.genderId === 2 
